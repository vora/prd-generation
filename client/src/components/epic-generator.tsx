...
import { Loader2, Sparkles, Target, Users, CheckCircle, ChevronDown, Trash2, MoreVertical, CheckSquare, Plus, Download } from "lucide-react";
...
export default function EpicGenerator({ prdId, prdTitle }: EpicGeneratorProps) {
  ...
  const handleDownloadEpics = () => {
    if (!hasEpics || !epicsData?.[0]?.content?.epics) {
      toast({
        title: "No epics to download",
        description: "Please generate epics first before downloading",
        variant: "destructive",
      });
      return;
    }

    const epics = epicsData[0].content.epics;
    const content = generateEpicsDocument(epics, prdTitle);
    const blob = new Blob([content], { type: 'text/plain' });
    const filename = `${prdTitle.toLowerCase().replace(/\s+/g, '-')}-epics.txt`;
    downloadBlob(blob, filename);
    
    toast({
      title: "Epics downloaded!",
      description: `Downloaded ${epics.length} epics with all user stories`,
    });
  };

  const generateEpicsDocument = (epics: any[], title: string) => {
    const timestamp = new Date().toLocaleDateString();
    let content = `# EPICS & USER STORIES\n`;
    content += `Generated for: ${title}\n`;
    content += `Date: ${timestamp}\n`;
    content += `Total Epics: ${epics.length}\n`;
    content += `\n${'='.repeat(80)}\n\n`;

    epics.forEach((epic, epicIndex) => {
      content += `## EPIC ${epicIndex + 1}: ${epic.title}\n\n`;
      content += `**Description:** ${epic.description}\n\n`;
      content += `**Priority:** ${epic.priority}\n`;
      content += `**Estimated Effort:** ${epic.estimatedEffort}\n\n`;
      
      if (epic.goals && epic.goals.length > 0) {
        content += `**Goals:**\n`;
        epic.goals.forEach((goal: string) => {
          content += `• ${goal}\n`;
        });
        content += '\n';
      }
      ...
      if (epic.userStories && epic.userStories.length > 0) {
        content += `**User Stories (${epic.userStories.length}):**\n\n`;
        epic.userStories.forEach((story: any, storyIndex: number) => {
          content += `### Story ${storyIndex + 1}: ${story.title}\n`;
          content += `**Description:** ${story.description}\n`;
          content += `**Priority:** ${story.priority}\n`;
          content += `**Status:** ${story.status}\n`;
          content += `**Story Points:** ${story.estimatedStoryPoints}\n\n`;
    ...
    content += `\n## SUMMARY\n`;
    content += `Total Epics: ${epics.length}\n`;
    content += `Total User Stories: ${epics.reduce((total, epic) => total + (epic.userStories?.length || 0), 0)}\n`;
