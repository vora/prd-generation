...
export default function ConversationRecorder({ onConversationComplete }: ConversationRecorderProps) {
  ...
  const updateConversationPhase = (currentTranscript: string) => {
    const wordCount = currentTranscript.split(' ').length;
    
    if (wordCount < 50) {
      setConversationPhase('intro');
      setCurrentPrompt(Math.floor(Math.random() * conversationPrompts.intro.length));
    } else if (wordCount < 200) {
      setConversationPhase('discovery');
      setCurrentPrompt(Math.floor(Math.random() * conversationPrompts.discovery.length));
    } else if (wordCount < 400) {
      setConversationPhase('details');
      setCurrentPrompt(Math.floor(Math.random() * conversationPrompts.details.length));
    } else {
      setConversationPhase('validation');
      setCurrentPrompt(Math.floor(Math.random() * conversationPrompts.validation.length));
    }
  };
        ...
        <CardContent>
          <div className="space-y-3">
            <div className="text-lg font-medium text-blue-900">
              {conversationPrompts[conversationPhase][currentPrompt]}
            </div>
            <div className="text-sm text-blue-700">
              <strong>Phase:</strong> {conversationPhase === 'intro' && 'Understanding the problem'} 
              {conversationPhase === 'discovery' && 'Exploring user needs and workflows'}
              {conversationPhase === 'details' && 'Gathering technical requirements'}
              {conversationPhase === 'validation' && 'Confirming and finalizing requirements'}
            </div>
            
            <Button 
              onClick={() => setCurrentPrompt((currentPrompt + 1) % conversationPrompts[conversationPhase].length)}
              variant="outline" 
              size="sm"
